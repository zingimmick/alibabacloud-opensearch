...
class DocumentServiceProcessor
{
    ...
    protected function process_push($seqid, $input, $output)
    {
        ...
        if ($bin_accel) {
            thrift_protocol_write_binary($output, 'push', \Thrift\Type\TMessageType::REPLY, $result, $seqid, $output->isStrictWrite());
        } else {
            $output->writeMessageBegin('push', \Thrift\Type\TMessageType::REPLY, $seqid);
            $result->write($output);
            $output->writeMessageEnd();
            $output->getTransport()->flush();
        }
